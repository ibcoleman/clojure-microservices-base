...
if [ -n "$PIDS" ]
then
  echo "Spa-om-clojure is already running in process $PIDS";
  exit 1
fi
...
LOG_FILE=${LOGGING_PATH}/spa-om-clojure.out
ERR_FILE=${LOGGING_PATH}/spa-om-clojure.err
...
echo "Giving Spa-om-clojure $waitTimeout seconds to start successfully"
...
do
  ...
  then
    echo "Spa-om-clojure didn't start within $waitTimeout seconds."
    PIDS=$(pgrep java -lf | grep $APP_NAME | cut -d" " -f1);
    if [ -n "$PIDS" ]
	then
	  echo "Killing $PIDS";
...
if [ $retVal -eq 1 ]
then
  echo "Starting Spa-om-clojure failed"
else
  echo "Starting Spa-om-clojure succeeded"
fi
